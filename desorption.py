...
import isosteric_heating as iso
obj=rootfind()
...
def eqns(t, vars):
    ...
    return [dT_dt, dw_dt]
# Initial conditions
T0 = iso.T_ads_start #itial temperature in Kelvin (20°C)
w0 = 0.3413     # Initial uptake (kg/kg)
initial_conditions = [T0, w0]
# Time span for the simulation (0 to 12 hours)
t_span =(iso.t_ads_start , 12*3600)
t_eval = np.linspace(t_span[0], t_span[1], 1000)
def event_min_uptake(t, y):
    ...
    return dw_dt
event_min_uptake.terminal = True
event_min_uptake.direction = 1
# Solve the system of ODEs
solution = solve_ivp(eqns, t_span, initial_conditions, t_eval=t_eval , method='Radau',events=event_min_uptake, rtol=1e-6, atol=1e-9)
...
T_switchdes = T_solution[w_min_index]
w_switchdes = w_solution[w_min_index]
t_switchdes = solution.t[w_min_index]
if __name__ == "__main__":
    ...
    print(f"Corresponding bed temperature at minimum uptake = {T_switchdes - 273.15:.2f} °C")
    # Plotting results
